...
type SendState = { ok: true; name: string } | { ok: false; error: string } | null;
...
export default function AboutPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (result.ok) {
      setSent({ ok: true, name: form.name.trim() });
    } else {
      setSent({ ok: false, error: result.error ?? 'Error desconocido' });
    }
  };
          ...
          <form className={'contact-form' + (shake ? ' shake' : '')} onSubmit={onSubmit}>
            {sent === null ? (
              ...
              <div className="terminal-success">
                ...
                <div className="term-body">
                  ...
                  <div className="line success">
                    &gt; MENSAJE RECIBIDO. TE RESPONDEREMOS PRONTO. GRACIAS, {sent.name.toUpperCase()}.<span className="caret">_</span>
                  </div>
              ...
              <div className="terminal-error">
                ...
                <div className="term-body">
                  <div className="line"><span className="prompt">vault@arcade:~$</span> ./send_message --to=team</div>
                  <div className="line dim">[ERR] Conectando con servidor…</div>
                  <div className="line error">
                    &gt; ERROR: {sent.error}<span className="caret">_</span>
                  </div>
                  <div style={{ marginTop: 18 }}>
                    <button className="btn ghost" type="button" onClick={reset}>REINTENTAR</button>
                  </div>
                </div>
              </div>
            )}
